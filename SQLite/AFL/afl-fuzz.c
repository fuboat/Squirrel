...
int main(int argc, char ** argv){
  ...
  //detect if bitmap changed (has_new_bit()), if so, update bitmap. Update testcase library
  //mutate testcase
  //move them into process queue

  queue<Testcase> tqueue = initial(); //initialize the initial testcase from file, allocate test queue
  setup_shm(); //initial 
  
  while(!tqueue.empty()){
    Testcase testcase = tqueue.pop_front();
    
    run_testcase(testcase.root);// call ir->translate(), fork to run testcase, record exec status
    unsigned int result = check_result(); //check exec result, check bitmap, update bitmap (by using has_new_bits())
    ...
    if(mutated_roots.empty()){
      re_mutated(); //if no new testcase generated, we need to randomly mutated again.
    }
